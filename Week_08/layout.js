...
function layout(element) {
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (itemStyle.flex) {
			flexLine.push(item)
		} else if (style.flexWrap == 'nowrap' && autoMainSize) {
	...
	if (style.flexWrap == 'nowrap' || autoMainSize) {
	...
	flexLines.forEach(flexLine => {
		...
		for (let i = 0; i < flexLine.length; i++) {
			...
			if (align == 'center') {
				itemStyle[crossStart] = crossBase + crossSign * (lineCrossSize - itemStyle[crossSpace]) / 2
				itemStyle[crossEnd] = itemStyle[crossStart] + crossSign * itemStyle[crossSize]
			}
			if (align == 'stretch') {
				itemStyle[crossStart] = crossBase
