		...
		if (!match(element, selectorParts[0])) continue
			...
			if (matched) {
				...
				for (let declaration of rule.declarations) {
					let p = declaration.property
					if (!computedStyle[p]) {
						computedStyle[p] = {}
					}
					if (!computedStyle[p].specificity) {
						computedStyle[p].value = declaration.value
						computedStyle[p].specificity = sp
					} else if (compare(computedStyle[p].specificity, sp) < 0) {
						computedStyle[p].value = declaration.value
						computedStyle[p].specificity = sp
